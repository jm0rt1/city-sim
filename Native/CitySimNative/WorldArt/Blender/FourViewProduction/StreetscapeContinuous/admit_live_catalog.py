#!/usr/bin/env python3
"""Admit the validated continuous camNE roads into the SwiftPM catalog."""
...
def main() -> None:
    if not VALIDATION.is_file() or not VALIDATION.read_text().startswith(
        ...
        raise RuntimeError("VALIDATED_CONTINUOUS_STREETSCAPE_REQUIRED")
    ...
    print(
        f"FOUR_VIEW_CONTINUOUS_ROAD_ADMISSION_PASS masks={len(roads)} output={OUTPUT}"
    )
