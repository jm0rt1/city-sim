...
- **Latest L4 disposition:** Matte candidate `6a717959` has correct pixels but is returned for byte-derived fail-closed admission. Custom semantic checkpoint `3822fa4b` is rejected from the candidate because it duplicates the renderer and produced no actual-scale proof. CONTRACT-019 R2 authorizes the smaller existing-renderer approach: repair matte admission, remove the custom rasterizer, then run exactly two diagnostic-only SceneKit/Metal semantic passes through the governed offline renderer. No authoritative raw process, B/C, siblings, normalization authority, ingestion, shipping, or production selection is authorized until independent approval of the actual-scale visibility proof.
