...
- **Latest L4 disposition:** CONTRACT-019 R5 reached `HARD_PIPELINE_LIMIT`, so SceneKit is closed for new source authority. The first CONTRACT-020 Blender process A preserved all 51 v18 components and materially improved the visible render, but was correctly rejected because a 1.5x landscape orthographic-scale mismatch clipped its contact field. The published R2 authority permits only the exact aspect-ratio camera correction, one fail-closed A probe, and conditional B/C identity proof. Portal redesign, siblings, normalization, ingestion, shipping, and production selection remain blocked until the corrected DCC calibration passes three-process decoded identity plus independent Renderer and QA review.
