...
V02_CONFIG = (
    ...
    "industrial-l04-v18-north-calibration-v02/CALIBRATION-CONTRACT.json"
)
EXPECTED_V02_CONFIG_SHA = (
...
def main():
    ...
    v02_config_path = inside(root, V02_CONFIG)
    v02_config = json.loads(v02_config_path.read_text(encoding="utf-8"))
    ...
    require(
        digest(v02_config_path) == EXPECTED_V02_CONFIG_SHA,
        "v02 calibration contract drift",
    )
    ...
    require(
        repair["shiftYFormula"]
        == "postProjectionOffsetPixels.y / renderViewportPixels.width",
        "shift y formula",
    )
    require(
        math.isclose(
    ...
    require(repair["groundPlaneCitySimY"] == 0, "ground plane")
    require(repair["originSource"] == [768, 768], "origin source")
    ...
    for field in ("descriptor", "materialLibrary", "comparisons", "expected", "cycles"):
        require(config[field] == v02_config[field], f"v02 {field} preservation")
    predecessor_blender = dict(v02_config["blender"])
    successor_blender = dict(config["blender"])
    predecessor_blender.pop("versionFile")
    successor_blender.pop("versionFile")
    require(
        successor_blender == predecessor_blender,
        "v02 Blender environment preservation",
    )
    renderer_text = script_path.read_text(encoding="utf-8")
    require(
        "registrationPlaneDerivation" not in renderer_text,
        "artificial registration-plane proof removed",
    )
    require(
        "groundPlaneCitySimY" in renderer_text,
        "actual ground-plane proof present",
    )
    report = {
        "schema": 1,
        "task": "PLAY-027",
        ...
        "predecessorConfigSHA256": digest(v02_config_path),
        ...
        "predecessorPreservation": {
            ...
            "actualGroundPlaneReplacesDerivedPlane": True,
        },
        "cycles": cycles,
