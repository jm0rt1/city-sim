#!/usr/bin/env python3
"""Admit the validated refined camNE roads into the SwiftPM resource catalog."""
...
def main() -> None:
    if not VALIDATION.is_file() or not VALIDATION.read_text().startswith(
        "STREETSCAPE_REFINED_VALIDATION_PASS"
    ):
        raise RuntimeError("VALIDATED_REFINED_STREETSCAPE_REQUIRED")
    ...
    print(f"FOUR_VIEW_REFINED_ROAD_ADMISSION_PASS masks={len(roads)} output={OUTPUT}")
