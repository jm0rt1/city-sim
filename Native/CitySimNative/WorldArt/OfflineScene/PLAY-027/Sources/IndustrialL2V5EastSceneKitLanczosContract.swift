...
enum IndustrialL2V5EastSceneKitLanczosContractError:
    Error,
...
struct IndustrialL2V5EastSceneKitLanczosRecord {
    let value: [String: Any]
    let supportGeometry: OversampledSupportGeometry
}

struct OversampledSupportGeometry: Equatable {
    let outputTargetCoordinate: [Int]
    let downsampledInputCoordinate: [Int]
    let highResolutionCenterTwice: [Int]
    let highResolutionWindowBoundsExclusive: [Int]
    let highResolutionWindowPixels: [Int]
    let linearOversamplingFactor: Int
    let capturedRadiusInputPixels: Int
}

enum IndustrialL2V5EastSceneKitLanczosContract {
    static let contractID =
        "industrial-l02-source-v05-east-scene-kit-vs-lanczos-707x687-v1"
    ...
    static let capturedRadiusInputPixels = 16
    ...
    static func supportGeometry(
        outputTargetCoordinate: [Int],
        viewportPixels: [Int],
        postProjectionOffsetPixels: [Double],
        linearOversamplingFactor: Int
    ) throws -> OversampledSupportGeometry {
        guard
            outputTargetCoordinate == targetCoordinate,
            viewportPixels == expectedViewportPixels,
            postProjectionOffsetPixels
                == expectedPostProjectionOffsetPixels,
            linearOversamplingFactor
                == expectedLinearOversamplingFactor
        else {
            throw IndustrialL2V5EastSceneKitLanczosContractError
                .invalid(
                    "SceneKit/Lanczos capture geometry must retain the frozen East camera and factor"
                )
        }
        let offsetX = Int(postProjectionOffsetPixels[0])
        let offsetY = Int(postProjectionOffsetPixels[1])
        let downsampledInputCoordinate = [
            outputTargetCoordinate[0] - offsetX,
            outputTargetCoordinate[1] - offsetY,
        ]
        guard
            downsampledInputCoordinate[0] >= 0,
            downsampledInputCoordinate[1] >= 0,
            downsampledInputCoordinate[0] < viewportPixels[0],
            downsampledInputCoordinate[1] < viewportPixels[1]
        else {
            throw IndustrialL2V5EastSceneKitLanczosContractError
                .invalid("mapped downsample input is outside the viewport")
        }

        // Inverse pixel-center mapping for scale 0.25:
        // highCenter = (outputPixel + 0.5) * 4 - 0.5.
        // Store twice the center to keep the contract integer-exact.
        let highResolutionCenterTwice = downsampledInputCoordinate.map {
            2 * $0 * linearOversamplingFactor
                + linearOversamplingFactor - 1
        }
        let floorCenter = highResolutionCenterTwice.map { $0 / 2 }
        let radius = capturedRadiusInputPixels
        let bounds = [
            floorCenter[0] - radius,
            floorCenter[1] - radius,
            floorCenter[0] + radius + 1,
            floorCenter[1] + radius + 1,
        ]
        let highResolutionPixels = viewportPixels.map {
            $0 * linearOversamplingFactor
        }
        guard
            bounds[0] >= 0,
            bounds[1] >= 0,
            bounds[2] <= highResolutionPixels[0],
            bounds[3] <= highResolutionPixels[1]
        else {
            throw IndustrialL2V5EastSceneKitLanczosContractError
                .invalid("captured support window is outside the 4x frame")
        }
        return OversampledSupportGeometry(
            outputTargetCoordinate: outputTargetCoordinate,
            downsampledInputCoordinate: downsampledInputCoordinate,
            highResolutionCenterTwice: highResolutionCenterTwice,
            highResolutionWindowBoundsExclusive: bounds,
            highResolutionWindowPixels: [
                bounds[2] - bounds[0],
                bounds[3] - bounds[1],
            ],
            linearOversamplingFactor: linearOversamplingFactor,
            capturedRadiusInputPixels: radius
        )
    }
    ...
    static func validate(
        ...
        guard requestedContractID == contractID else {
            throw IndustrialL2V5EastSceneKitLanczosContractError.invalid(
                "unknown East SceneKit/Lanczos contract"
            )
        }
        guard
            ...
            throw IndustrialL2V5EastSceneKitLanczosContractError.invalid(
                "SceneKit/Lanczos capture requires frozen Industrial L2 source-v05 East"
            )
        }
        guard
            ...
            throw IndustrialL2V5EastSceneKitLanczosContractError.invalid(
                "SceneKit/Lanczos capture requires explicit none/current/current and coordinate 707,687"
            )
        }
        guard
            sampling.contractID
            ...
            throw IndustrialL2V5EastSceneKitLanczosContractError.invalid(
                "SceneKit/Lanczos capture sampling contract drifted"
            )
        }
        guard
            ...
            throw IndustrialL2V5EastSceneKitLanczosContractError.invalid(
                "SceneKit/Lanczos input path mismatch"
            )
        }
        guard
            ...
            throw IndustrialL2V5EastSceneKitLanczosContractError.invalid(
                "SceneKit/Lanczos capture requires a new exact PLAY-027 run-a, run-b, or run-c directory"
            )
        }

        let geometry = try supportGeometry(
            outputTargetCoordinate: targetCoordinate,
        ...
        return IndustrialL2V5EastSceneKitLanczosRecord(
            value: [
                ...
                "purpose":
                    "East 707,687 SceneKit 4x versus software Lanczos isolation only",
