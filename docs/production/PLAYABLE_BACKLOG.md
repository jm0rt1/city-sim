...
- **Latest L4 gate:** CONTRACT-020 R3 is integrated at `f63a504e`: actual CitySim `y = 0` ground registration, three-process decoded raw/semantic identity, complete text-authority reconstruction, and deterministic evidence pass. Calibration capability is accepted; the unchanged v18 art remains returned at literal 192. The North art-v01 authority now requires a stepped foundry silhouette, dominant unobstructed freight portal, stronger facade/roof/material hierarchy, restrained contact field, and at least 64 deterministic Cycles samples. World Art must stop after one A render for independent art-direction review. Siblings, normalization, R3 ingestion, shipping, and production selection remain blocked.
