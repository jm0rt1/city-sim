...
def configure_camera(descriptor):
    ...
    # Blender expresses both camera shifts against the viewport width. Using
    # the height here magnifies the landscape registration by the 1.5 aspect.
...
def build_projection_proof(scene, camera, descriptor):
    ...
    # Registration authority is the actual CitySim ground plane. No derived or
    # elevated plane may be solved from the expected pixels.
    registration_plane_y = 0.0
    ...
    expected_origin = descriptor["camera"]["sourceGroundCenter"]
    origin_world = [0.0, registration_plane_y, 0.0]
    actual_origin = source_pixel(scene, camera, origin_world)
    ...
    origin_error = distance(actual_origin, expected_origin)
    ...
    edge_contact = any(
        ...
        for point in actual_footprint + [actual_pivot, actual_socket, actual_origin]
    )
    passed = (
        all(max(error) <= 1.0 for error in footprint_errors)
        and max(pivot_error) <= 1.0
        and max(socket_error) <= 1.0
        and max(origin_error) <= 1.0
    ...
    proof = {
        ...
        "groundPlaneCitySimY": registration_plane_y,
        "groundPlaneAuthority": "actual-configured-citysim-y-zero",
        ...
        "originExpectedSource": expected_origin,
        "originWorldXYZ": origin_world,
        "originActualSource": actual_origin,
        "originAbsoluteError": origin_error,
