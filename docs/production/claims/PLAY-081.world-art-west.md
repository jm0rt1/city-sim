...
- **Worktree:** `/Users/James/.codex/worktrees/ef17/city-sim`
...
- **Status:** Predesign accepted and integrated at `8a889f2a`. Direction-local
  ...
  `73cd3d408ff6cb65e8137f26f8d25c1f42f7a81d` and proves
...
Own only the West schedule-consumer contract/consumer/tests,
`west_execution_orchestration_v2.py`,
`WEST-EXECUTION-ORCHESTRATION-V2.json`, `run_west_source.py`,
`RUNNER-CONTRACT.json`, and West-exclusive closure evidence inside the existing
revision-2 exclusive roots. Bind the exact trusted-master schedule and
authenticated one-attempt authority through the named high-level orchestrator
to the runner's validation-only boundary.
...
Update only current West task-root orchestrator/runner contracts, validators,
