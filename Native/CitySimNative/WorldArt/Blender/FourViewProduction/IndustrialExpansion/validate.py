#!/usr/bin/env python3
"""Independently validate the Copperline source and clean-rerender determinism."""
...
spec = importlib.util.spec_from_file_location("citysim_industrial_validator", SERVICES_VALIDATOR)
...
def validate_industrial_geometry(asset):
    root = bpy.data.objects.get("AssetRoot")
    require(root is not None, "MISSING_ASSET_ROOT", asset["assetId"])
    require(root.get("industrialRole") == asset["industrialRole"], "INDUSTRIAL_ROLE_MISMATCH", asset["assetId"])
    require(root.get("assetKind") == "industrial-low", "ASSET_KIND_MISMATCH", asset["assetId"])
...
def validate_preview():
    ...
    require({placement["assetId"] for placement in data["placements"]} == {"copperline_machine_shop", "ironleaf_service_workshop"}, "PREVIEW_ASSET_SET_MISMATCH", path)
...
def main():
    ...
    results = {
        "schema": "citysim.world-art.industrial-expansion-validation.v1",
        "status": "PASS",
        "blenderVersion": actual,
        "assets": {},
    }
    with tempfile.TemporaryDirectory(prefix="citysim-industrial-expansion-determinism-") as temp_dir:
        temp_root = Path(temp_dir)
        for asset in CONFIG["assets"]:
            mesh_count, axes = common.validate_scene(asset)
            validate_industrial_geometry(asset)
    ...
    report = "INDUSTRIAL_EXPANSION_FOUR_VIEW_VALIDATION_PASS\n" + json.dumps(results, indent=2, sort_keys=True) + "\n"
