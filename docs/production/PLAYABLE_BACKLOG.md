...
- **Latest L4 gate:** Matte R2 remains approved. R4 proved the residual 143-pixel drift is a portal-joint preparation-state split, and Renderer localized it to intentionally overlapping lintel/header and north-jamb geometry rather than the physically separated crucible. CONTRACT-019 R5 authorizes one exact `0.0625` shader-only header depth-ownership rule plus exactly two diagnostic semantic processes. Failure is a hard SceneKit pipeline limit; success repairs determinism only. Portal modeling and all raw/sibling/normalization/R3-ingestion/shipping/production-selection work remain blocked.
