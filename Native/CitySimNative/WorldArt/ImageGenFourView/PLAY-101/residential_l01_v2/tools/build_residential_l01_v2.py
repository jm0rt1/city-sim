#!/usr/bin/env python3
"""Build deterministic keyed LODs and family contact sheets for residential_l01_v2."""
...
SOUTH_RAW = REPO / "Native/CitySimNative/WorldArt/ImageGenSingleAngle/PLAY-097/residential/raw/residential_l01_variant_2/source-v01.png"
SOUTH_LODS = {
    lod: REPO / f"Native/CitySimNative/WorldArt/ImageGenSingleAngle/PLAY-097/residential/normalized/residential_l01_variant_2/{lod}/source-v03.png"
...
def raw_path(direction: str) -> Path:
    if direction == "south":
        return SOUTH_RAW
    return FAMILY / f"raw/{direction}-source-v01.png"
...
def build_outputs(output_root: Path) -> dict[str, object]:
    ...
    return {
        "schema": "citysim.play-097.residential-l01-v2.build.v1",
        ...
        "legacySouthLods": {
            lod: {
                ...
                "sha256": sha256(path),
                "disposition": "preserved immutable predecessor; not used by this family packet",
            }
            for lod, path in SOUTH_LODS.items()
        },
    }


def main() -> int:
    record = build_outputs(FAMILY)
    receipt = FAMILY / "BUILD-RECEIPT.json"
