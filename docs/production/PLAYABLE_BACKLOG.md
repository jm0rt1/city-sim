...
- **Latest L4 gate:** CONTRACT-020 R2 proved three-process decoded raw/semantic identity, complete 51-component/13-material transfer, clean alpha, and metadata-only PNG container differences. Renderer returned the pipeline because its validator projected an artificial `y = 11.4309845` plane while the governed ground remained 64 source pixels low; QA separately approved the visible pipeline capability and returned the current art at literal 192. R3 authorizes only the exact ground-plane correction (`shift_y = 128 / 1536`, proof at CitySim `y = 0`) and renewed A/B/C identity proof. Portal redesign, siblings, normalization, R3 ingestion, shipping, and production selection remain blocked until the R3 DCC calibration passes independent Renderer and QA review.
