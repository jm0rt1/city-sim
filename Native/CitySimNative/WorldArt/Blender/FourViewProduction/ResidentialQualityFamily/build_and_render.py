...
def residential_palette(prefix, siding, accent, roof, masonry):
    return {
        "grass": material(prefix + "GardenGround", (0.31, 0.42, 0.22, 1), texture_scale=7.0, bump=0.11),
        "walk": material(prefix + "WarmWalk", (0.72, 0.62, 0.48, 1), texture_scale=12.0, bump=0.14),
        "foundation": material(prefix + "FoundationStone", masonry, texture_scale=14.0, bump=0.20),
        "wall": material(prefix + "Siding", siding, texture_scale=16.0, bump=0.13),
        "wallDark": material(prefix + "SidingShadow", tuple(value * 0.69 for value in siding[:3]) + (1,), texture_scale=17.0, bump=0.14),
        "trim": material(prefix + "IvoryTrim", (0.92, 0.84, 0.68, 1), texture_scale=6.0, bump=0.05),
        "roof": material(prefix + "Roof", roof, roughness=0.54, texture_scale=22.0, bump=0.22),
        "roofEdge": material(prefix + "RoofEdge", tuple(min(1.0, value * 1.22) for value in roof[:3]) + (1,), roughness=0.50, texture_scale=18.0, bump=0.12),
        "glass": material(prefix + "WindowGlass", (0.18, 0.43, 0.53, 1), roughness=0.22, texture_scale=0),
        "door": material(prefix + "Door", accent, roughness=0.48, texture_scale=8.0, bump=0.12),
        "brick": material(prefix + "ChimneyBrick", (0.55, 0.22, 0.12, 1), texture_scale=18.0, bump=0.21),
        "wood": material(prefix + "PorchWood", (0.43, 0.24, 0.12, 1), texture_scale=11.0, bump=0.16),
        "iron": material(prefix + "Iron", (0.15, 0.17, 0.16, 1), roughness=0.43, metallic=0.50, texture_scale=8.0),
        "leaf": material(prefix + "Leaf", (0.18, 0.40, 0.15, 1), texture_scale=7.0, bump=0.10),
        "leaf2": material(prefix + "LeafLight", (0.38, 0.55, 0.20, 1), texture_scale=6.0, bump=0.08),
        "flower": material(prefix + "Flower", accent, roughness=0.62, texture_scale=0),
    }
...
def window_y(root, name, x, y, z, mats, width=0.48, height=0.64, shutters=False):
    box(root, name + "Glass", (x, y, z), (width, 0.055, height), mats["glass"], edge=0.009)
    box(root, name + "Top", (x, y - 0.016, z + height / 2 + 0.055), (width + 0.15, 0.075, 0.10), mats["trim"], edge=0.006)
    box(root, name + "Bottom", (x, y - 0.016, z - height / 2 - 0.045), (width + 0.13, 0.075, 0.08), mats["trim"], edge=0.006)
    if shutters:
        for side in (-1, 1):
            box(root, name + ("LeftShutter" if side < 0 else "RightShutter"), (x + side * (width / 2 + 0.09), y - 0.012, z), (0.12, 0.07, height + 0.04), mats["door"], edge=0.008)


def window_x(root, name, x, y, z, mats, width=0.48, height=0.64, shutters=False):
    box(root, name + "Glass", (x, y, z), (0.055, width, height), mats["glass"], edge=0.009)
    box(root, name + "Top", (x - 0.016, y, z + height / 2 + 0.055), (0.075, width + 0.15, 0.10), mats["trim"], edge=0.006)
    box(root, name + "Bottom", (x - 0.016, y, z - height / 2 - 0.045), (0.075, width + 0.13, 0.08), mats["trim"], edge=0.006)
    if shutters:
        for side in (-1, 1):
            box(root, name + ("NearShutter" if side < 0 else "FarShutter"), (x - 0.012, y + side * (width / 2 + 0.09), z), (0.07, 0.12, height + 0.04), mats["door"], edge=0.008)


def door_y(root, name, x, y, z, mats, double=False):
    width = 0.86 if double else 0.56
    box(root, name + "Recess", (x, y + 0.035, z), (width + 0.18, 0.14, 1.28), mats["wallDark"], edge=0.018)
    box(root, name + "Door", (x, y - 0.055, z), (width, 0.075, 1.16), mats["door"], edge=0.018)
    box(root, name + "Transom", (x, y - 0.062, z + 0.72), (width, 0.05, 0.20), mats["glass"], edge=0.006)
    box(root, name + "Lintel", (x, y - 0.070, z + 0.88), (width + 0.26, 0.10, 0.12), mats["trim"], edge=0.008)
...
def planter(root, prefix, x, y, mats, scale=0.24, flowers=False):
    ...
    if flowers:
        for index, offset in enumerate((-0.16, 0.0, 0.16)):
            sphere(root, f"{prefix}Flower{index}", (x + offset, y - 0.19, 0.43), (0.065, 0.055, 0.075), mats["flower"], subdivisions=1)
...
def alder_gable_cottage(root):
    m = residential_palette("Alder", (0.68, 0.50, 0.28, 1), (0.20, 0.42, 0.44, 1), (0.12, 0.23, 0.27, 1), (0.48, 0.42, 0.34, 1))
    ...
    window_y(root, "AlderRearLeft", -0.62, 1.425, 1.48, m, shutters=True)
    window_y(root, "AlderRearRight", 0.60, 1.425, 1.48, m, shutters=True)
    window_x(root, "AlderEastFront", 1.425, -0.30, 1.48, m, shutters=True)
    window_x(root, "AlderEastRear", 1.425, 0.72, 1.48, m, shutters=True)
    ...
    for index, (x, y) in enumerate(((-1.52, -1.48), (1.50, -1.48), (-1.50, 1.46), (1.50, 1.46))):
        planter(root, f"AlderGarden{index}", x, y, m, flowers=index < 2)
...
def birch_lane_bungalow(root):
    m = residential_palette("Birch", (0.74, 0.69, 0.50, 1), (0.58, 0.26, 0.16, 1), (0.20, 0.29, 0.25, 1), (0.56, 0.46, 0.34, 1))
    ...
    for index, x in enumerate((-1.00, 0, 1.00)):
        window_y(root, f"BirchRear{index}", x, 1.535, 1.38, m, width=0.52, height=0.66, shutters=True)
    for side, x in (("West", -1.575), ("East", 1.575)):
        for index, y in enumerate((-0.38, 0.56)):
            window_x(root, f"Birch{side}{index}", x, y, 1.38, m, width=0.54, height=0.66, shutters=True)
    ...
    for index, (x, y) in enumerate(((-1.60, -1.55), (1.58, -1.55), (-1.62, 1.48), (1.60, 1.48))):
        planter(root, f"BirchGarden{index}", x, y, m, scale=0.25, flowers=index != 2)


def rosewood_turret_house(root):
    m = residential_palette("Rosewood", (0.55, 0.28, 0.22, 1), (0.18, 0.43, 0.38, 1), (0.12, 0.20, 0.25, 1), (0.47, 0.38, 0.32, 1))
    ...
    for floor, z in enumerate((1.34, 2.62), start=1):
        ...
        window_y(root, f"RosewoodRearF{floor}", -0.30, 1.505, z, m, width=0.48, height=0.66, shutters=True)
    ...
    for index, (x, y) in enumerate(((-1.58, -1.52), (1.58, -1.54), (-1.58, 1.48), (1.54, 1.46))):
        planter(root, f"RosewoodGarden{index}", x, y, m, scale=0.25, flowers=index in (0, 1, 3))


def stonebridge_duplex(root):
    m = residential_palette("Stonebridge", (0.67, 0.52, 0.34, 1), (0.24, 0.38, 0.52, 1), (0.16, 0.23, 0.25, 1), (0.44, 0.37, 0.31, 1))
    ...
    for side, x in (("West", -0.58), ("East", 0.58)):
        ...
        for floor, z in enumerate((1.36, 2.52), start=1):
            ...
            window_y(root, f"Stonebridge{side}RearF{floor}", x, 1.525, z, m, width=0.48, height=0.64, shutters=True)
    for side_name, x in (("West", -1.625), ("East", 1.625)):
        for floor, z in enumerate((1.36, 2.52), start=1):
            for bay, y in enumerate((-0.34, 0.66)):
                window_x(root, f"Stonebridge{side_name}F{floor}B{bay}", x, y, z, m, width=0.46, height=0.62, shutters=floor == 2)
    for side, x in (("West", -0.92), ("East", 0.92)):
        ...
        window_y(root, f"Stonebridge{side}SunroomRear", x, 1.645, 1.18, m, width=0.56, height=0.64)
    ...
    for index, (x, y) in enumerate(((-1.58, -1.56), (1.58, -1.56), (-1.58, 1.48), (1.58, 1.48))):
        planter(root, f"StonebridgeGarden{index}", x, y, m, scale=0.23, flowers=index < 2)
...
def baseline_comparison_sheet(asset_paths):
    ...
    height = header + image_size + gap + header + image_size
    rgba = bytearray(bytes((24, 29, 29, 255)) * (width * height))
    ...
    paste_row(quality, header + image_size + gap, ("QUALITY", "FAMILY"))
    output = HERE / CONFIG["output"]["baselineComparison"]
...
def write_family_manifest(asset_manifests, aggregate_path, comparison_path):
    data = {
        ...
        "admittedBaselineComparison": artifact_info(comparison_path, HERE),
        "sourceFiles": [source_info(HERE / name) for name in ("build_and_render.py", "pipeline.json", "run_pipeline.sh", "validate.py")],
    }
...
def main():
    ...
    comparison_path = baseline_comparison_sheet(asset_paths)
