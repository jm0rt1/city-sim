...
struct CityBenchmarkReportService {
    ...
    private func availableURL() -> URL {
        var suffix = 1
        var candidate = reportDirectoryURL.appending(path: "native-vertical-slice-v1.json")
        while fileManager.fileExists(atPath: candidate.path) {
            suffix += 1
            candidate = reportDirectoryURL.appending(path: "native-vertical-slice-v1-\(suffix).json")
        }
        return candidate
    }
}
