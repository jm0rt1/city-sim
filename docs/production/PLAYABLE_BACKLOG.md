...
- **Latest L4 gate:** V16 and v17 contain the identical 1,807 contaminated coordinates, localizing the defect to the opaque-magenta/contact-shadow/partial-alpha compositor boundary rather than modeling. Integration authorizes only a two-run no-Metal matte-attribution diagnostic under `docs/production/evidence/PLAY-027/INDUSTRIAL-L04-V17-RAW-RETURN-AND-MATTE-DIAGNOSTIC-AUTHORITY.md`. No further modeling, raw process, B/C, siblings, normalization authority, R3 ingestion, shipping, or production selection is authorized.
