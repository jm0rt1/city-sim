...
private struct FamilyRaster {
    ...
    let medianLuma: Int
    let p95Luma: Int
    let saturatedAccentPixelCount: Int
    let saturatedAccentShare: Double
}
...
private struct DirectionFiles {
    ...
    let baselineNormalizedRoot: String
}
...
private let outputRelative =
    "docs/production/evidence/PLAY-027/industrial-l03/l03/"
    + "cohesion-a0-family-v01/chroma-repair-v01/final-review"
...
private func alphaData(_ rgba: Data) -> Data {
...
private func changedRGBPixelCount(_ lhs: Data, _ rhs: Data) -> Int {
    let left = [UInt8](lhs)
    let right = [UInt8](rhs)
...
private func straightChannel(_ channel: Int, alpha: Int) -> Int {
...
private func inspect(_ target: URL) throws -> FamilyRaster {
    ...
    for y in 0..<image.height {
        for x in 0..<image.width {
            ...
            let straightRed = straightChannel(red, alpha: alpha)
            let straightGreen = straightChannel(green, alpha: alpha)
            let straightBlue = straightChannel(blue, alpha: alpha)
            visible += 1
            ...
            let luma =
                (54 * straightRed + 183 * straightGreen
                    + 19 * straightBlue + 128) >> 8
            lumas.append(luma)
            let maximum = max(straightRed, straightGreen, straightBlue)
            let minimum = min(straightRed, straightGreen, straightBlue)
            if maximum >= 160, maximum - minimum >= 96 {
                saturatedAccents += 1
            }
        }
    }
    ...
    let p95 = lumas.isEmpty ? 0 : lumas[p95Index]
    let bounds =
    ...
    return FamilyRaster(
        ...
        zeroAlphaPixelCount: zeroAlpha,
        hiddenRGBPixelCount: hidden,
        exactChromaPixelCount: chroma,
        visibleMagentaSpillPixelCount: spill,
        medianLuma: median,
        p95Luma: p95,
        saturatedAccentPixelCount: saturatedAccents,
        saturatedAccentShare:
            visible == 0 ? 0 : Double(saturatedAccents) / Double(visible)
    )
}
...
private func numberArray2D(
    _ value: Any?,
    label: String
) throws -> [[Double]] {
    guard let values = value as? [[NSNumber]] else {
        throw IndustrialL3CohesionFamilyReviewError.invalid(
            "\(label) numeric matrix missing"
        )
    }
...
private func compactOverlay(
    raster: FamilyRaster,
    descriptor: [String: Any],
    normalization: [String: Any]
) throws -> (image: CGImage, record: [String: Any]) {
    guard
        let descriptorRegistration =
            descriptor["registration"] as? [String: Any],
        let normalizerRegistration =
            normalization["registration"] as? [String: Any],
        let uniformScale =
            (normalizerRegistration["uniform_scale"] as? NSNumber)?
                .doubleValue
    else {
        throw IndustrialL3CohesionFamilyReviewError.invalid(
            "overlay registration contract missing"
        )
    }
    let sourceBounds = try numberArray(
        normalizerRegistration["source_bbox"],
        label: "normalizer source bounds"
    )
    let targetOrigin = try numberArray(
        normalizerRegistration["target_origin"],
        label: "normalizer target origin"
    )
    let frontageEdge = try numberArray2D(
        descriptorRegistration["frontageEdgeSource"],
        label: "frontage edge"
    )
    let doorBase = try numberArray2D(
        descriptorRegistration["doorBaseSource"],
        label: "door base"
    )
    let socket = try numberArray(
        descriptorRegistration["frontageSocketSource"],
        label: "frontage socket"
    )
    guard
        sourceBounds.count == 4,
        targetOrigin.count == 2,
        frontageEdge.count == 2,
        doorBase.count == 2,
        socket.count == 2
    else {
        throw IndustrialL3CohesionFamilyReviewError.invalid(
            "overlay registration dimensions invalid"
        )
    }
    func project(_ point: [Double]) -> [Double] {
        let registeredX =
            targetOrigin[0] + (point[0] - sourceBounds[0]) * uniformScale
        let registeredY =
            targetOrigin[1] + (point[1] - sourceBounds[1]) * uniformScale
        return [
            registeredX * Double(raster.image.width) / 1536.0,
            registeredY * Double(raster.image.height) / 1024.0,
        ]
    }
    let projectedEdge = frontageEdge.map(project)
    let projectedDoor = doorBase.map(project)
    ...
    let socketDistance = hypot(
        socketMidpoint[0] - projectedSocket[0],
        socketMidpoint[1] - projectedSocket[1]
    )
    return (
        try imageFromRGBA(
            pixels,
        ...
        [
            ...
            "frontageEdgeNeighborhood": projectedEdge,
            "doorBaseColor": "white",
            "doorBaseNeighborhood": projectedDoor,
            "doorBaseWidthPixels": doorWidth,
            "socketColor": "yellow-cross",
            "socketNeighborhood": projectedSocket,
            "doorBaseMidpointToSocketPixels": socketDistance,
            "frontageWidthAtLeastEightPixels":
                doorWidth >= 8,
            "socketRegistrationWithinTwoPixels":
                socketDistance <= 2,
            "authority":
                "descriptor registration transformed through exact run-a normalization registration",
        ]
    )
}
...
private func inspectCGImage(_ image: CGImage) throws -> FamilyRaster {
    ...
    return FamilyRaster(
        ...
        medianLuma: 0,
        p95Luma: 0,
        saturatedAccentPixelCount: 0,
        saturatedAccentShare: 0
    )
}
...
enum BuildIndustrialL3CohesionFamilyReview {
    static func main() throws {
        ...
        for files in directions {
            ...
            let normalizationProvenanceAPath =
                "\(files.normalizedRoot)/run-a/\(files.direction)/provenance.json"
            let normalizationProvenanceBPath =
                "\(files.normalizedRoot)/run-b/\(files.direction)/provenance.json"
            let normalizationProvenanceA = try json(
                url(normalizationProvenanceAPath, root: root)
            )
            let normalizationProvenanceB = try json(
                url(normalizationProvenanceBPath, root: root)
            )
            guard
                normalizationProvenanceA["strict_chroma_contract"] as? String
                    == StrictNonzeroAlphaChromaCanonicalizer.contractID,
                normalizationProvenanceB["strict_chroma_contract"] as? String
                    == StrictNonzeroAlphaChromaCanonicalizer.contractID,
                normalizationProvenanceA["productionSelected"] as? Bool
                    == false,
                normalizationProvenanceB["productionSelected"] as? Bool
                    == false
            else {
                throw IndustrialL3CohesionFamilyReviewError.invalid(
                    "\(files.direction) strict normalization provenance failed"
                )
            }
            var directionLODRecords: [[String: Any]] = []
            var compactRaster: FamilyRaster?
            for (lod, expectedDimensions) in lods {
                ...
                let baselinePath =
                    "\(files.baselineNormalizedRoot)/run-a/"
                    + "\(files.direction)/\(filename)"
                let baselineRaster = try inspect(url(baselinePath, root: root))
                let alphaIdentity =
                    alphaData(rasterA.rgba) == alphaData(baselineRaster.rgba)
                let changedRGBPixels = changedRGBPixelCount(
                    baselineRaster.rgba,
                    rasterA.rgba
                )
                let passed =
                ...
                normalizedRecords.append([
                    ...
                    "alphaSHA256": sha256(alphaData(rasterA.rgba)),
                    "baselineAlphaIdentity": alphaIdentity,
                    "strictRepairChangedRGBPixelCount": changedRGBPixels,
                    "visiblePixelCount": rasterA.visiblePixelCount,
                    "zeroAlphaPixelCount": rasterA.zeroAlphaPixelCount,
                    "repeatFileIdentity": true,
                    "repeatDecodedPixelIdentity": true,
                    "hiddenRGBPixelCount": 0,
                    "exactChromaPixelCount": 0,
                    "nearChromaAtNonzeroAlphaPixelCount": 0,
                    "medianLuma": rasterA.medianLuma,
                    "p95Luma": rasterA.p95Luma,
                    "saturatedAccentPixelCount":
                        rasterA.saturatedAccentPixelCount,
                    "saturatedAccentShare":
                        rasterA.saturatedAccentShare,
                    "paddingPassed": true,
                    "registrationPassed": true,
                ])
                directionLODRecords.append([
                    "lod": lod,
                    "medianLuma": rasterA.medianLuma,
                    "p95Luma": rasterA.p95Luma,
                    "saturatedAccentPixelCount":
                    ...
                    "saturatedAccentShare":
                        rasterA.saturatedAccentShare,
                    "nonzeroAlphaPixelCount": rasterA.visiblePixelCount,
                    "zeroAlphaPixelCount": rasterA.zeroAlphaPixelCount,
                    "zeroAlphaHiddenRGBPixelCount":
                        rasterA.hiddenRGBPixelCount,
                    "exactChromaAtNonzeroAlphaPixelCount":
                        rasterA.exactChromaPixelCount,
                    "nearChromaAtNonzeroAlphaPixelCount":
                        rasterA.visibleMagentaSpillPixelCount,
                ])
            }
            guard let compactRaster else {
                throw IndustrialL3CohesionFamilyReviewError.invalid(
                    "\(files.direction) compact raster missing"
                )
            }
            let overlay = try compactOverlay(
                raster: compactRaster,
                descriptor: descriptor,
                normalization: normalizationProvenanceA
            )
            overlayImages.append(overlay.image)
            var overlayRecord = overlay.record
            overlayRecord["direction"] = files.direction
            overlayRecords.append(overlayRecord)
            directionMetrics.append([
                "direction": files.direction,
                "lods": directionLODRecords,
                "saturatedAccentDefinition":
                    "straight maxRGB >= 160 and maxRGB-minRGB >= 96 at alpha > 0",
                "lumaDefinition":
                    "straight sRGB integer Rec.709 approximation (54R+183G+19B+128)>>8 at alpha > 0",
            ])
        }
        guard
            rawHashes.count == 4,
        ...
        let panelSpecs: [(String, CGImage, String)] = [
            ...
            (
                "COMPACT-NESW-ROAD-SOCKET-OVERLAY.png",
                try canvas(
                ...
                "N/E/S/W row-major literal neighborhood LOD; cyan authoritative road edge, white door/loading-base segment, yellow socket cross"
            ),
            (
        ...
        let strictCanonicalizerSourcePath =
            "Native/CitySimNative/WorldArt/OfflineScene/PLAY-027/Tools/"
        ...
        let manifest: [String: Any] = [
            ...
            "returnedCandidate":
                "03b0ffc1211eb8d4cbefa5676671b004cc03605c",
            "publishedRepairAuthority":
                "ac14f7684bd090d55bf7b9920810fefdb3260191",
            ...
            "strictNonzeroAlphaChromaContract":
                StrictNonzeroAlphaChromaCanonicalizer.contractID,
            "normalizerSource": normalizerSourcePath,
            "normalizerSourceSHA256": try sha256(
                url(normalizerSourcePath, root: root)
            ),
            "strictCanonicalizerSource": strictCanonicalizerSourcePath,
            "strictCanonicalizerSourceSHA256": try sha256(
                url(strictCanonicalizerSourcePath, root: root)
            ),
            "strictNonzeroAlphaChromaAccounting": directionMetrics,
            "registrationPivotSocketFrontageShadow": "pass",
            "compactRoadSocketOverlay": overlayRecords,
            "compactFrontageVisualInspection": [
                [
                    "direction": "north",
                    "result": "FAIL_ART",
                    "finding":
                        "authoritative socket and door-base segment align to the far road edge, but foreground mass occludes the loading faces; only dark upper slivers under the ochre fascia survive",
                ],
                [
                    "direction": "east",
                    "result": "PASS",
                    "finding":
                        "three full dark loading faces and the separate staff entrance survive at compact scale on the authoritative road edge",
                ],
                [
                    "direction": "south",
                    "result": "PASS",
            ...
            "reviewStatus":
                "blocked-north-west-compact-frontage-art-failure",
        ...
        **Disposition:** `BLOCKED_NORTH_WEST_COMPACT_FRONTAGE_ART_FAILURE`.
        ...
        The strict task-owned post-resize canonicalizer repairs the returned
        normalization defect: all twelve LODs are unique, exact across two
        runs, and contain zero exact or near chroma at every nonzero alpha,
        with alpha, padding, registration, and accepted catalog bytes
        preserved.

        The literal compact-neighborhood road/socket overlay reveals an art
        defect rather than a proof omission. East and South retain three full
        dark loading faces plus the staff entrance. North and West place the
        authoritative cyan road edge, white door-base segment, and yellow
        socket correctly, but foreground mass hides the loading faces; only
        dark upper slivers beneath the ochre fascia survive. The candidate
        therefore stops before geometry mutation exactly as required by the
        published return. Source authority, family authority, and
        `productionSelected` remain false.
        """
        try FileManager.default.createDirectory(
            at: output,
            withIntermediateDirectories: true
