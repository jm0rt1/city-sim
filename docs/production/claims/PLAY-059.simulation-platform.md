# PLAY-059 Claim

- **Title:** Give every diagnostic layer authoritative local truth
- **Lane:** Simulation platform
- **Branch:** `codex/citysim-simulation-platform`
- **Worktree:** `/Users/James/.codex/worktrees/e909/city-sim`
- **Base authority:** First published integration authority containing this claim
- **Claimed:** July 25, 2026
- **Planned surfaces:** `CitySpatialConsequences`, immutable presentation snapshot tests, focused deterministic/performance evidence, and `docs/production/evidence/PLAY-059/`
- **Dependencies:** accepted existing spatial consequence platform; CONTRACT-013; exact published claim baseline
- **Validation/proof:** exact repeat samples for story states; applicability and monotonicity fixtures; snapshot/save/fingerprint compatibility; focused and full native suites; measured construction budget
- **Status:** dispatched after the claim baseline is published; acceptance belongs to integration

Add only the three optional transient channels approved by CONTRACT-013:
developed-tile Land Value, developed-tile Local Happiness, and road-tile
Traffic Pressure. Derive them from existing authoritative state without
changing simulation outcomes, save bytes, fingerprints, commands, stores, or
renderer code.

Do not invent measured trips/congestion, rebalance gameplay, edit SpriteKit or
SwiftUI, change schemas, regenerate authentic legacy fixtures, push, integrate,
self-accept, or pin the thread. Commit product/tests and evidence/completion as
separate coherent outcomes.
