...
- **Latest L4 gate:** Exact diagnostic `e8c1ba77c4ed10193a7ff53cdc7a07cab7ae989d` owns and safely removes all 1,807 compositor-created near-chroma pixels, but the v17 SceneKit result still collapses toward the rejected v16 narrow-opening composition at native-2x and literal 192. World Art must first implement the versioned `matte-canonicalization-v2` regression and an exact-camera per-component `semantic-visibility-v1` proof under CONTRACT-019. No authoritative raw process, B/C, siblings, normalization authority, R3 ingestion, shipping, or production selection is authorized until independent Renderer and QA approval.
