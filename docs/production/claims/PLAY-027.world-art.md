...
- **Latest L4 disposition:** Matte R2 remains approved. R3 v18 removed the duplicate foundation and reduced semantic A/B drift from 13,629 to 143 pixels, but repeat identity still fails; portal header support also changes from 19 to 17 literal pixels. CONTRACT-019 R4 authorizes only a two-replay no-Metal edge-locality analyzer on retained R3 outputs to distinguish SceneKit preparation from raster-edge/resolve behavior. Portal modeling and every further SceneKit/Metal/raw/sibling/normalization/ingestion/shipping/production-selection action remain blocked.
