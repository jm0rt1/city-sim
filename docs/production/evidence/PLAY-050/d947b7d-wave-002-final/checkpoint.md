...
- **Status:** Superseded by the independently reproduced rejection in `disposition.md`
...
- Two compact post-dismissal frames were retained for the focus-routing defect. The fresh controlled default and compact reproductions are recorded as `PLAY-050-D006` in this directory.

## Limitations after rejection

- The complete 87-test suite and exact staged identity subsequently passed and are recorded in `automated-validation.md` and `manifest.md`.
- D001 and the executed D002 route passed, then D006 rejected the candidate. The 32-command traversal, persistence/recovery, later accessibility variants, full RSS comparison, and 20-minute journey remain uncredited as listed in `session-record.md`.
