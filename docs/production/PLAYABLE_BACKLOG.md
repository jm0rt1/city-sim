...
- **Latest L4 gate:** Matte R2 `d6347a74` is independently approved and existing-renderer diagnostic `a47215ea` is retained as return evidence. Its A/B split is caused by exact coplanar duplicate foundations; the portal separately fails with a three-pixel literal-192 south jamb. CONTRACT-019 R3 authorizes one new North descriptor revision removing only redundant `v16-foundation` plus exactly two diagnostic semantic processes. Portal modeling and all authoritative raw, sibling, normalization, R3 ingestion, shipping, and production-selection work remain blocked until repeat identity passes independently.
